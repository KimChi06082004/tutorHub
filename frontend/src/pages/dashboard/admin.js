    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="flex justify-center gap-4 mb-10 flex-wrap">
          {[
            ["users", "👤 Người dùng", "blue"],
            ["classes", "📚 Lớp học", "green"],
            ["tutors", "🎓 Gia sư", "amber"],
          ].map(([key, label, color]) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`px-5 py-2.5 rounded-lg font-semibold border transition-all duration-200 shadow-sm
                ${
                  tab === key
                    ? `bg-${color}-600 text-white border-${color}-700 shadow-md`
