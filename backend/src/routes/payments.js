...
router.post("/stripe", async (req, res) => {
  try {
    ...
    // Stripe chỉ hỗ trợ USD
    const amountInUSD = amount / 24000;
    const stripeAmount = Math.round(amountInUSD * 100);

    // ✅ Dùng domain thật khi deploy
    const frontendURL = process.env.FRONTEND_URL || "http://localhost:3000";
    console.log("🌍 FRONTEND_URL hiện tại:", frontendURL);
...
router.get("/all", verifyToken, requireRoles(["admin"]), async (req, res) => {
  try {
    ...
    let sql = `
      SELECT payment_id, order_id, payment_method, amount, status, transaction_code, created_at
      FROM payments
      WHERE status = 'SUCCESS'
    `;
    const params = [];
